...
public class LowPriorityKafkaConsumer extends AbstractKafkaEventConsumer<String, LowPriorityMessage> {
    ...
    public LowPriorityKafkaConsumer (KafkaListenerEndpointRegistry endpointRegistry) {
        super();
    ...
    @KafkaListener(id = "lowPriorityMessageListener", topics = "${wynk.kafka.consumers.listenerFactory.low[0].factoryDetails.topic}", containerFactory = "${wynk.kafka.consumers.listenerFactory.low[0].name}")
    protected void listenLowPriorityMessage(@Header(value = StreamConstant.MESSAGE_LAST_ATTEMPTED_SEQUENCE, required = false) String lastAttemptedSequence,
                                            ...
                                            @Header(value = StreamConstant.RETRY_COUNT, required = false) String retryCount,
                                             ConsumerRecord<String, LowPriorityMessage> consumerRecord) {
        try {
            ...
            if (!(e instanceof WynkRuntimeException)) {
                log.error(StreamMarker.KAFKA_POLLING_CONSUMPTION_ERROR, "Something went wrong while processing message {} for kafka consumer : {}", consumerRecord.value(), ", LowPriorityMessage - ", e);
            }
        }
    }

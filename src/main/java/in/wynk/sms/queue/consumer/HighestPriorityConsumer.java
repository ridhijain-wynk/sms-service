...
@Slf4j
public class HighestPriorityConsumer extends AbstractSQSMessageConsumerPollingQueue<HighestPriorityMessage> {

    @Value("${sms.priority.highest.queue.consumer.enabled}")
    private boolean enabled;
    @Value("${sms.priority.highest.queue.consumer.delay}")
    private long consumerDelay;
    @Value("${sms.priority.highest.queue.consumer.delayTimeUnit}")
    private TimeUnit delayTimeUnit;

    private final ExecutorService messageHandlerThreadPool;
    private final ScheduledExecutorService pollingThreadPool;

    public HighestPriorityConsumer(String queueName,
                                AmazonSQS sqs,
    ...
    @Override
    @TimeIt
    @AnalyseTransaction(name="highestConsumer")
    ...
    @Override
    public Class<HighestPriorityMessage> messageType() {
        return HighestPriorityMessage.class;
    }

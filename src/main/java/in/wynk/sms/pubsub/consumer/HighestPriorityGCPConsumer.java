...
@Slf4j
public class HighestPriorityGCPConsumer  extends AbstractPubSubMessagePolling<HighestPriorityGCPMessage> {

    @Value("${sms.priority.highest.pubSub.consumer.enabled}")
    private boolean enabled;

    @Value("${sms.priority.highest.pubSub.consumer.delay}")
    private long consumerDelay;
    @Value("${sms.priority.highest.pubSub.consumer.delayTimeUnit}")
    ...
    @Override
    public void consume(HighestPriorityGCPMessage message) {
        try {
    ...
    @Override
    public Class<HighestPriorityGCPMessage> messageType() {
        return HighestPriorityGCPMessage.class;
    }

    @Override
    public void start() {
        if (enabled) {
            log.info("Starting HighestPriorityGCPConsumer...");

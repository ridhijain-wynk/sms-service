...
public class SendersCachingService implements IEntityCacheService<Senders, String> {
    ...
    public Senders getSenderByNameClientCountry(String name, String clientAlias, SMSPriority priority, String countryCode) {
        List<Senders> list =  SENDER_BY_IDS_CACHE.values().stream().filter(senders -> senders.getClientAlias().equalsIgnoreCase(clientAlias)
                && senders.getName().equalsIgnoreCase(name)
                && senders.getCountryCode().equalsIgnoreCase(countryCode)).collect(Collectors.toList());

...
public class SenderConfigurationsCachingService implements IEntityCacheService<SenderConfigurations, String> {
    ...
    private final Map<String, SenderConfigurations> SENDER_CONFIG_BY_ALIAS_CACHE = new ConcurrentHashMap<>();
    ...
    public void loadSenderConfigurations() {
        ...
        if (CollectionUtils.isNotEmpty(senderConfigurations) && writeLock.tryLock()) {
            ...
            Map<String, SenderConfigurations> clientAliasMap = new ConcurrentHashMap<>();
            try {
                senderConfigurations.forEach(config -> {
                    clientAliasMap.put(config.getClientAlias(), config);
                    configIdsMap.put(config.getId(), config);
                });
                SENDER_CONFIG_BY_ALIAS_CACHE.clear();
                SENDER_CONFIG_BY_IDS_CACHE.clear();
                SENDER_CONFIG_BY_ALIAS_CACHE.putAll(clientAliasMap);
    ...
    public SenderConfigurations getSenderConfigurationsByAlias(String alias) {
        return StringUtils.isNotEmpty(alias) ? SENDER_CONFIG_BY_ALIAS_CACHE.get(alias) : null;
    }
